...
function _inner_cholesky_factorize!(dim, V, das)
    z = zero(eltype(V))
    V[1] = sqrt(V[1])
    for r in 2:dim
        kr = das[r]-r
...
function cholesky_factorize!(A::MT) where {MT<:SkylineMatrix}
    V = A.coefficients
    das = A.das
    _inner_cholesky_factorize!(A.dim, V, das)
end

function cholesky_solve(A::MT, rhs) where {MT<:SkylineMatrix}
    n=A.dim
    V = A.coefficients
    das = A.das
    b = deepcopy(rhs)
